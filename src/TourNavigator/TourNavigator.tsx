...
import { Align, Position, FitPriority, TourNavigatorProps, TourNavigatorStates, Step, ClientBoundingRect, HelperProps, RecursiveFunction, IntersectionOption } from './types'
...
class TourNavigator extends PureComponent<TourNavigatorProps, TourNavigatorStates> {
    ...
    getDynamicIntersectionOption(): IntersectionObserverInit | undefined {
        ...
        let currentIntersectionOption: IntersectionOption | undefined;

        if(typeof this.currentStep?.intersectionOption === 'function'){
            currentIntersectionOption = this.currentStep?.intersectionOption({...intersectionOption})
        }else {
            currentIntersectionOption = this.currentStep?.intersectionOption
        }

        if(currentIntersectionOption?.rootMargin){
            intersectionOption.rootMargin = currentIntersectionOption.rootMargin
        }

        if(currentIntersectionOption?.threshold){
            intersectionOption.threshold = currentIntersectionOption.threshold
        }

        if(typeof currentIntersectionOption?.root === 'string'){
            intersectionOption.root = document.querySelector(currentIntersectionOption?.root)
        }else if(currentIntersectionOption?.root){
            intersectionOption.root = currentIntersectionOption?.root
        }
